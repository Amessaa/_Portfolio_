...
let model = null
gltfLoader.load(
    '/models/planet/planet/scene.gltf',
    (gltf) =>
    {
        model = gltf.scene
...
let modelAdded, skilladded = false
window.addEventListener('scroll',()=>{
    scrollY  = window.scrollY-0.5
    
    cameraGroup.position.y = Math.sin(cameraGroup.position.x + scrollY/window.innerHeight)
    cameraGroup.position.z = Math.cos(cameraGroup.position.x + scrollY/window.innerHeight)

        
    // const skillscontainer = document.getElementById('scene-container');
    // const sctop = skillscontainer.offsetTop+1500;
    // const scbottom = sctop + window.innerHeight;

    // if(scrollY >= sctop && scrollY <= scbottom){
    //     if(!skilladded){
    //         initSkillCubes();
    //         skilladded=true;
    //     }
